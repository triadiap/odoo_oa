...
class ExcavaMiniApprovalWizard(models.TransientModel):
    _name = 'excavamini.approval.wizard'
    _description = 'Excavamini Report Approval Wizard'

    id_excavamini_report = fields.Many2one('oa.excavaminidaily.report', string='Related ID')
    ...
    def default_get(self, fields):
        res = super(ExcavaMiniApprovalWizard, self).default_get(fields)
        if self._context.get('active_id'):
            model = self.env['oa.excavaminidaily.report'].browse(self._context.get('active_id'))
            res['id_excavamini_report'] = model.id
            ...
            res['equip_id'] = model.excavamini_equip_id
        return res
    ...
    def action_confirm(self):
        ...
        if next_step:
            ...
            self.env['oa.excavaminidaily.report'].generate_excavamini_tasklist(
                self.equip_id
            )
        records = self.env['oa.excavaminidaily.report'].browse(self.id_excavamini_report.id)
        records.write(vals)
