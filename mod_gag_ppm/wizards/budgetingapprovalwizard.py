...
class TransientBudgetingApproval(models.TransientModel):
    _name = 'budgeting.approval.wizard'
    _description = 'Budgeting Approval Wizard'

    id_budget = fields.Many2one('informasi.perpillar', string='Related Budget ID')
    ...
    def default_get(self, fields):
        res = super(TransientBudgetingApproval, self).default_get(fields)
        if self._context.get('active_id'):
            model = self.env['informasi.perpillar'].browse(self._context.get('active_id'))
    ...
    def action_confirm(self):
        ...
        if next_step:
            vals = {
                ...
                'keterangan_anggaran':self.approval_note
            }
        else:
            vals = {
                'state':'approve',
        ...
        records = self.env['informasi.perpillar'].browse(self.id_budget.id)
        records.write(vals)
