...
class MoCutiTahunan(models.Model):
    _name = "mo.cuti.tahunan"
    ...
    tujuan_cuti = fields.Selection([
        ("tahunan", "Cuti Tahunan"),
        ("melahirkan", "Cuti Melahirkan"),
        ("lainnya", "Keterangan Lain")
    ], string="Tujuan Cuti", default="tahunan", required=True)
    ...
    def create(self, vals):
        ...
        if config:
            ...
            next_step = self.env['oa.document.workflow.config'].search([('model_id.model', '=', self._name)], limit=1).approval_route_id.step_ids.sorted(key='sequence')[0]
        ...
        return super(MoCutiTahunan, self).create(vals)
    ...
    def action_approval_1(self):
        for record in self:
            ...
            records = self.env['mo.cuti.tahunan'].browse(self.id)
            records.write(vals)
